...
export default function ReceiverPage() {
  ...
  const [selectedSender, setSelectedSender] = useState<Listing | null>(null);
  ...
  const loadListings = async (latitude: number, longitude: number) => {
    const response = await apiFetch<Listing[]>(
      `/api/listings?latitude=${latitude}&longitude=${longitude}&radiusKm=3`
    );
  ...
  useEffect(() => {
    navigator.geolocation?.getCurrentPosition(
      (position) => {
        ...
        loadListings(nextCoordinates.latitude, nextCoordinates.longitude).catch(
          () => {
      ...
      () => {
        loadListings(coordinates.latitude, coordinates.longitude).catch(() => {
          toast.error("Unable to load nearby listings.");
        });
      }
    );
  ...
  const handleRequest = (sender: Listing) => {
  ...
  const confirmRequest = async () => {
    ...
    try {
      ...
      await apiFetch("/api/requests", {
        method: "POST",
        body: JSON.stringify({
          listing_id: selectedSender.id,
          urgency,
          location_label: "Receiver live location",
  ...
  const urgencyConfig = {
    ...
    medium: {
      color: "bg-[#FFE5D9] text-[#FF6B35] border-[#FF6B35]",
      icon: AlertTriangle,
      label: "Need by today",
    },
    high: {
  ...
  const filteredSenders = useMemo(
    () =>
      listings.filter(
        (sender) =>
          sender.providerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
          sender.location.toLowerCase().includes(searchQuery.toLowerCase())
      ),
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 space-y-6">
          <Card className="border-slate-200 shadow-lg bg-white overflow-hidden">
            <div className="p-1 bg-gradient-to-r from-[#FF6B35] to-[#FFC857]" />
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-slate-800 flex items-center justify-between">
              Available Near You
              <Badge className="bg-slate-100 text-slate-600 hover:bg-slate-200 shadow-none border border-slate-200">
                {filteredSenders.length} Found
              </Badge>
            </h3>
            ...
            {filteredSenders.map((sender) => (
              <Card
                key={sender.id}
                className="border-slate-200 shadow-sm hover:shadow-md transition-shadow group overflow-hidden"
              >
                <CardContent className="p-0">
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="flex items-center text-sm font-bold text-[#4CAF50] bg-[#4CAF50]/10 px-2 py-0.5 rounded-md">
                            <Star className="h-3.5 w-3.5 mr-1 fill-[#4CAF50]" />
                            {sender.trustScore}
                          </span>
                      ...
                      <Badge
                        variant="outline"
                        className="border-[#FF6B35] text-[#FF6B35] bg-[#FFE5D9]/30 font-semibold px-2 py-1"
                      >
                        <Clock className="h-3 w-3 mr-1" />
                        {new Date(sender.availableFrom).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </Badge>
          ...
          <Card className="h-full border-slate-200 shadow-lg overflow-hidden flex flex-col">
            <CardHeader className="bg-slate-50 border-b border-slate-100 py-3 flex-none">
              <CardTitle className="flex justify-between items-center text-base">
                <span className="flex items-center gap-2">
                  <MapPin className="h-5 w-5 text-[#FF6B35] fill-[#FF6B35]/20" />
                  3km Radius Map
                </span>
            ...
            <CardContent className="p-0 flex-1 relative">
              ...
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden">
                <div
                  className="w-[150%] h-[150%] rounded-full border border-[#FF6B35]/20 animate-ping"
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setShowRequestDialog(false)}
      >
          ...
          <div className="p-6">
            <DialogHeader className="mb-4">
              ...
              <DialogDescription className="text-slate-500 font-medium pt-1">
                You are requesting an LPG cylinder exchange with{" "}
                {selectedSender?.providerName}.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-4 animate-in fade-in zoom-in-95">
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-[#FF6B35]">Exchange Only</span>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 bg-blue-50 text-blue-800 rounded-lg border border-blue-100">
                  ...
                  <p className="text-xs font-medium leading-relaxed">
                    By confirming, your verified profile and trust score will be
            ...
            {requestStatus === "waiting" && (
              <div className="py-12 flex flex-col items-center justify-center space-y-4 animate-in fade-in">
                <div className="relative">
                  <div className="w-16 h-16 rounded-full border-4 border-slate-100 border-t-[#FF6B35] animate-spin" />
            ...
            {requestStatus === "confirmed" && (
              <div className="py-8 flex flex-col items-center justify-center space-y-4 animate-in zoom-in-90 fade-in">
                ...
                <div className="text-center text-slate-600 bg-slate-50 p-4 rounded-xl border border-slate-100 w-full mb-2">
                  <p className="font-bold mb-1">{selectedSender?.location}</p>
                  <p className="text-sm">
                    The provider can now accept or reject your request.
                  </p>
                </div>
                <Button
                  variant="outline"
